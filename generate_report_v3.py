...
Cách dùng: python3 generate_report_v3.py
"""
import os
...
def _ensure_styles(doc):
    """Tạo các style cần thiết nếu chưa có (khi dùng file mẫu trường)."""
    styles = doc.styles
    # List Bullet
    try:
        styles['List Bullet']
...
def add_page_number(doc):
    ...
    Sử dụng OxmlElement để tương thích LibreOffice."""
    total_sections = len(doc.sections)
    last_section_idx = total_sections - 1
    for i, sec in enumerate(doc.sections):
        ...
        if i < last_section_idx:
            # Tất cả section trước section cuối: không có số trang
            continue

        # Section 2 trở đi: Nội dung chính => có số trang
        # Tạo paragraph properties để căn giữa
        ...
        # === Dùng complex field (fldChar) với OxmlElement ===
        # Run 1: fldChar BEGIN
        ...
        # Run 4: Gia tri mac dinh
        run4 = OxmlElement('w:r')
        rPr4 = OxmlElement('w:rPr')
        noProof = OxmlElement('w:noProof')
        rPr4.append(noProof)
        rFonts4 = OxmlElement('w:rFonts')
        rFonts4.set(qn('w:ascii'), FONT_NAME)
        rFonts4.set(qn('w:hAnsi'), FONT_NAME)
        rPr4.append(rFonts4)
...
# ============ MAIN ============
def main():
    template_path = os.path.join(SCRIPT_DIR, "MAU_KHOA_TTNN.docx")

    if os.path.exists(template_path):
        print("📄 Đang dùng file mẫu trường (MAU_KHOA_TTNN.docx) làm nền...")
        doc = Document(template_path)

        # Xóa nội dung từ Mục lục trở đi (P[44] trở đi) trong section 1
        # Section 0 = 2 trang bìa (giữ nguyên khung viền)
        # Từ P[44] trở đi = nội dung cũ cần thay thế
        body = doc.element.body
        paragraphs = body.findall(qn('w:p'))
        tables = body.findall(qn('w:tbl'))

        # Tìm section break paragraph (P[43] - ranh giới section 0 và 1)
        # Giữ lại tất cả elements trong section 0 (bìa)
        # Xóa tất cả elements trong section 1 (nội dung cũ)
        section_break_found = False
        elements_to_remove = []
        for child in list(body):
            tag = child.tag.split('}')[-1] if '}' in child.tag else child.tag
            if tag == 'sectPr':
                continue  # Giữ section properties cuối cùng
            if section_break_found:
                elements_to_remove.append(child)
            # Tìm section break trong paragraph
            if tag == 'p':
                pPr = child.find(qn('w:pPr'))
                if pPr is not None:
                    sect = pPr.find(qn('w:sectPr'))
                    if sect is not None and not section_break_found:
                        section_break_found = True
                        # Đánh dấu paragraph này cũng cần xóa (nó là ranh giới)

        # Xóa tất cả nội dung từ sau section break
        for elem in elements_to_remove:
            body.remove(elem)

        print(f"  ✅ Đã giữ nguyên {'' if section_break_found else 'toàn bộ'} trang bìa mẫu")
    else:
        print("⚠️  Không tìm thấy MAU_KHOA_TTNN.docx, tạo file mới...")
        doc = Document()
        sec = doc.sections[0]
        sec.top_margin = Cm(2)
    ...
    # Đảm bảo styles cần thiết tồn tại
    _ensure_styles(doc)
    ...
    # ===== Mục lục (không có số trang) =====
    # Khi dùng template: section break cho phần bìa đã có sẵn từ P[43]
    # Khi tạo mới: cần thêm section break
    if not os.path.exists(template_path):
        _add_section_break(doc)
    toc_page(doc)

    # ===== Nội dung chính (có số trang từ 1) =====
    sec_content = _add_section_break(doc)
    _reset_page_number(sec_content)
    ...
    # Chèn số trang + ngắt liên kết footer
    add_page_number(doc)

    out = "bao_cao_thuc_tap_v3.docx"
    doc.save(out)
    print(f"✅ Đã tạo: {out}")
    print("📌 Nếu footer hiện {{PAGE}} thay vì số: nhấn Alt+F9 trong WPS Office")
    print("📌 Nhấn Ctrl+A → F9 để cập nhật mục lục")
